document.addEventListener('DOMContentLoaded', function() {
    ...
    const GITHUB_TOKEN = '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';
    ...
    async function cargarDatos() {
        try {
            ...
            fileSha = data.sha;
            const contenidoDecodificado = decodeURIComponent(escape(atob(data.content)));
            reuniones = contenidoDecodificado ? JSON.parse(contenidoDecodificado) : [];
    ...
    async function guardarDatos() {
        try {
            ...
            const contenidoCodificado = unescape(encodeURIComponent(contenido));
            const contenidoBase64 = btoa(contenidoCodificado);
